...
welcome: {
  ...
  image: "/welcome-image.svg",         // Optional path to image
...
subflows: [
  {
    id: "dashboard-tour",
    type: "simple",
    name: "Explore the dashboard",      // Name shown in checklist
    description: "Learn about your dashboard", // Description shown in checklist
    page: "/dashboard",                 // Page where this tour activates
    buttonText: "Start tour",           // Button text in checklist
    steps: [
      // Step configuration (same format as simple flows)
    ]
  }
]
```
...
```typescript
showChecklist: true,                // Whether to show the checklist
checklistConfig: {
  header: "Getting started 👋",     // Checklist title
  position: "right",                // Position on screen
  minimizable: true,                // Allow users to minimize
}
```
...
Like simple flows, composite flows can be triggered either manually or automatically. See our [Triggers](https://docs.pointer.so/flows/triggers) page for details.
