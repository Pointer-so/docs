...
Unlike comprehensive [onboarding flows](https://docs.pointer.so/flows/composite-flow), simple flows tackle one specific goal—like creating a project, setting up a profile, or understanding a dashboard. They provide contextual guidance right when and where users need it.
...
steps: [
  {
    ...
    type: "guided",              // The type of step, use "guided"
...
* `select` — wait for the user to select an option from a dropdown

* `submit` — wait for a form submission

* `navigate` — wait for navigation to a specific page
...
You can trigger simple flows in two ways:

1. **Manual triggers** — Start flows on demand when users click buttons or interact with your UI

2. **Automatic triggers** — Start flows automatically based on conditions you define

For complete details on both triggering methods, see our [Triggers](https://docs.pointer.so/flows/simple-flow) page.
