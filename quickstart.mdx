...
There are two props you can pass to the `PointerProvider`:

1. `disabled?`: A boolean that enables/disables the Pointer widget (optional).
2. `apiUrl?`: A string that specifies the API URL to use (optional).

<Note>
  Note that the `apiUrl` will only work if you're using `api.pointer.so`. This prop exists as we will eventually transition to open-source. By default, you should not set this prop.
</Note>
